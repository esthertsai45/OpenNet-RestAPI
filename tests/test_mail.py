...
class TestMail:
    @pytest.mark.parametrize("email", [generate_valid_email(True),
                                       generate_valid_email(False)])
    def test_valid_mail_format(self, mail_client, email):
        ...
        assert res.json()[
            'format'] == True, "expected format is True. But got False"

    @pytest.mark.parametrize("email", [(lambda: generate_valid_email(is_disposable=True))() for _ in range(3)])
    def test_disposable_mail(self, mail_client, email):
        ...
        assert res.json()[
            'format'] == True, "expected format is True. But got False"
        assert res.json()[
            'disposable'] == True, "expected disposable is True. But got False"

    @pytest.mark.parametrize("email", [(lambda: generate_valid_email(is_disposable=False))() for _ in range(3)])
    def test_non_disposable_mail(self, mail_client, email):
        ...
        assert res.json()[
            'format'] == True, "expected format is True. But got False"
        assert res.json()[
            'disposable'] == False, "expected disposable is False. But got True"

    
    @pytest.mark.parametrize("email", [(lambda: generate_invalid_email_format())() for _ in range(3)])
    def test_invalid_special_characters(self, mail_client, email):
        res = mail_client.verify_email(email=email)

        assert res.status_code == 200, res.text
        assert res.json()['format'] == False, res.json()['format']